...
public class CartHandler implements CartStandard {
	...
	public List<Cart> fetchAllCarts() {
		...
		try {
			...
			while (rs.next()) {
				...
				list.add(new Cart(rs.getInt("id"), rs.getInt("userId"), rs.getInt("productId"), rs.getInt("quantity")
	...
	public Cart fetchProduct(int id) {
		try {
			...
			while (rs.next()) {
				return new Cart(rs.getInt("id"), rs.getInt("productId"), rs.getInt("userId"), rs.getInt("quantity"));
