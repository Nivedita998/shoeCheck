...
public class ProductHandler implements ProductStandard {
	...
	public List<Product> fetchAllProducts() {
		...
		try {
			...
			String query = "select p.*,c.name as catName,c.description as catDesc ,"
					+ "ib.url as imageUrl from product p,file c ,imageBank ib where "
	...
	public Product fetchProduct(int id) {
		try {
			...
			String query = "select p.*,c.name as catName,c.description as catDesc , ib.url as imageUrl from product p,file c ,imageBank ib where c.id=p.categoryid and ib.id=p.imageId and p.id = ?";
			Connection conn = Configuration.conn;
			PreparedStatement p = conn.prepareStatement(query);
			...
			// return new Product(rs.getInt("id"), rs.getString("name"), rs.getString("discription"),
